...
class TestPlotter:
    @mock.patch('plotter.plt')
    def test_accepts_correct_dataframe(self, mock_plt):
        df = pd.DataFrame({
    ...
    @mock.patch('plotter.plt')
    def test_plot_has_two_lines_correct_colors(self, mock_plt):
        df = pd.DataFrame({
    ...
    @mock.patch('plotter.plt')
    def test_axes_labeled_correctly(self, mock_plt):
        df = pd.DataFrame({
            'year_month': ['2008-01', '2008-02'],
            'question_count': [100, 150]
        })
        plot_questions(df)
        mock_plt.xlabel.assert_called_with('Month')
        mock_plt.ylabel.assert_called_with('Questions')

    @mock.patch('plotter.plt')
    def test_rolling_average_calculated_correctly(self, mock_plt):
        df = pd.DataFrame({
